...
OUT_BASE="${OUT_BASE:-${ROOT_DIR}/data/hierarchy/single_gate_1m}"
PREFIX_BASE="${PREFIX_BASE:-crbn_l1m}"

# Tier sizes
L1_REPS="${L1_REPS:-1000000}"
...
# Selection for single-gate expansion (after L1 screening)
L1_TOP_PERCENT="${L1_TOP_PERCENT:-10}"

# Optional screening on deeper tiers
SCREEN_L2="${SCREEN_L2:-false}"
SCREEN_L3="${SCREEN_L3:-false}"

# Clustering scaffold mode
...
screen_layer() {
  ...
  local prefix="${PREFIX_BASE}_layer${layer}"
...
select_keys() {
  ...
  local prefix="crbn_l1m_layer${layer}"
...
echo "[single-gate] L1 build (${L1_REPS} reps)"
build_layer 1 "${BITS_L1}" "${L1_REPS}" "" ""
screen_layer 1
L1_KEYS=$(select_keys 1 "${L1_TOP_PERCENT}")

L1_PARENTS=$(wc -l < "${L1_KEYS}")
L2_REPS=$((L1_PARENTS * PER_PARENT))
echo "[single-gate] L2 build: parents=${L1_PARENTS} reps=${L2_REPS}"
build_layer 2 "${BITS_L2}" "${L2_REPS}" "${BITS_L1}" "${L1_KEYS}"

if [[ "${SCREEN_L2}" == "true" ]]; then
  screen_layer 2
  L2_KEYS=$(select_keys 2 "${L2_TOP_PERCENT:-100}")
else
  cut -f2 "${OUT_BASE}/layer2/rep_meta.tsv" | tail -n +2 > "${OUT_BASE}/layer2/all_keys.txt"
  L2_KEYS="${OUT_BASE}/layer2/all_keys.txt"
fi

L2_PARENTS=$(wc -l < "${L2_KEYS}")
L3_REPS=$((L2_PARENTS * PER_PARENT))
echo "[single-gate] L3 build: parents=${L2_PARENTS} reps=${L3_REPS}"
build_layer 3 "${BITS_L3}" "${L3_REPS}" "${BITS_L2}" "${L2_KEYS}"

if [[ "${SCREEN_L3}" == "true" ]]; then
  screen_layer 3
fi

echo "[single-gate] done"
